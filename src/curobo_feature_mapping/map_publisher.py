...
Motion planning (added on top of the mapper; ``enable_planning`` param, on by
default). The same node owns a cuRobo ``MotionPlanner`` so it shares the in-GPU
map as the collision world (mesh re-extracted before each plan):

- ``~/move_to_pose`` (curobo_feature_mapping_interfaces/MoveToPose, action):
  plan a collision-free arm trajectory to a Cartesian tool pose OR to the
  centroid of a feature-map text query (``prompt``), and optionally execute it
  on the ``joint_trajectory_controller``. Feedback streams plan/exec phase,
  progress, and planned-vs-actual tracking error.
- ``~/plan_to_pose`` (curobo_feature_mapping_interfaces/PlanToPose, service):
  plan-only, blocking variant returning the trajectory (handy for CLI tests).

Planning needs the interfaces package built/sourced and a cuRobo robot config
matching the arm (``robot_config`` param; a bundled ``config/ur5e.yml`` is the
default, generated by ``scripts/gen_robot_cfg.py``). Execution forwards to
``control_msgs/action/FollowJointTrajectory`` on ``controller_action``.
...
# Motion planning lives in planning_node.PlanningServer, composed by this node
# (same process, shared in-GPU map). The module guards its own optional deps
# (cuRobo motion modules, control_msgs, the planning interfaces), so this import
# is always safe; create_planning_server returns None when planning is off or
# unavailable.
from curobo_feature_mapping.planning_node import create_planning_server
...
class CuroboMapPublisher(Node):
    def __init__(self):
        ...
        # --- motion planning ---
        p("enable_planning", True)
        p("robot_config", "")  # "" -> bundled config/ur5e.yml
        p("tool_frame", "tool0")
        p("joint_states_topic", "/joint_states")
        p("controller_action", "/joint_trajectory_controller/follow_joint_trajectory")
        p("plan_standoff", 0.15)  # m above a queried object centroid
        p("plan_max_velocity_scale", 1.0)  # 0..1 time-scaling of the executed traj
        p("plan_collision_activation_distance", 0.02)  # m safety margin around the map
        p("grasp_clearance", 0.03)  # m above the queried surface for the grasp pose
                                    # (the object is in the collision map; without a
                                    # gripper the tool stops just above it)
        ...
        # --- motion planning (optional; shares this node's in-GPU map) ---
        # PlanningServer owns the cuRobo planner + the move_to_pose/plan_to_pose
        # interfaces; it pulls the world via _extract_mesh_np and resolves
        # prompts via _run_query (both injected, so it stays decoupled).
        self._planning = create_planning_server(
            self,
            world_frame=self._world_frame,
            device=str(self._device),
            tf_buffer=self._tf_buffer,
            mesh_provider=self._extract_mesh_np,
            query_fn=self._run_query,
        )

        self.get_logger().info(
            f"Mapping {self._camera_frame} in {self._world_frame}; "
            f"features={'on' if self._radio else 'off'}; "
            f"planning={'on' if self._planning else 'off'}"
        )
    ...
    def _extract_mesh_np(self):
        # Safe before any frames: callers (planning world refresh) may ask early.
    ...
    def _run_query(self, prompt: str, top_k: int | None = None,
        ...
        # Top of the matched region (max world z): a top-down grasp/standoff aims
        # relative to the object surface, not its mid-volume centroid.
        result["top_z"] = round(float(centers[:, 2].max()), 4)
